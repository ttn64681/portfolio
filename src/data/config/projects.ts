...
export const projectsConfig: ProjectConfig[] = [
  {
    ...
    bullets: [
      'Led a 5-person Scrum/SWE team in Jira; improved sprint velocity ~10% via refined stories, modeling, and layered architecture.',
      'Architected Spring Boot + PostgreSQL with 80+ REST endpoints and 15 JPA entities; Dockerized backend and deployed to Render.',
      'Cut API payloads ~30–45% with DTOs, pagination, and facade/proxy patterns; JWT + Spring Security RBAC with BCrypt/AES protections.',
    ],
    techStack: ['Next.js 16', 'Spring Boot 3.5', 'PostgreSQL', 'Docker', '✚'],
    date: 'Aug 2025 – Feb 2026',
    link: 'https://acm-movies.vercel.app/',
    github: 'https://github.com/ttn64681/Absolute-Cinema-Movies',
    exploreDossier: {
      overview:
        'Layered full-stack cinema product spanning authentication, catalog search, seat orchestration, promotions, payments, orders, and admin workflows. I owned architecture direction, Spring Security integration, caching/pagination strategy, and frontend API boundaries while running Scrum across four sprints. Backend was containerized with Docker and deployed to Render.',
      features: [
        ...
        'Dockerized Spring backend deployed to Render.',
      ],
      implementation: [
        ...
        'Implemented Virtual Proxy/lazy-loading style responses in targeted flows to reduce initial payload overhead (~30–45%).',
        ...
        'Containerized the backend with Docker and deployed to Render.',
        'Established Jira process, team ticketing discipline, and dev-guide conventions for multi-sprint consistency; refined user stories and system modeling to improve sprint velocity ~10%.',
      ],
      challenges: [
  ...
  },
  {
    ...
    summary:
      'This site: Gemini RAG chat, Redis vectors, Dockerized Proxmox staging via self-hosted Actions, and a pixel UI.',
    bullets: [
      'Built a RAG chat pipeline with Gemini embeddings, Upstash Redis, and Vercel AI SDK streaming grounded by cosine similarity + prompt engineering.',
      'Deployed Dockerized staging on a Proxmox Ubuntu VM via CI/CD with a self-hosted GitHub Actions runner (lint/typecheck + RAG sync gates).',
      'Managed AI costs with IP rate limiting, query embedding caching, and SHA-256 hashing; troubleshot Linux DNS/vNIC, Windows NAT, and Docker port bindings.',
    ],
    techStack: ['Next.js 15', 'Gemini', 'Docker', 'GitHub Actions', '✚'],
    date: 'Jun 2025 – Present',
    github: 'https://github.com/ttn64681/portfolio',
    exploreDossier: {
      overview:
        'Personal portfolio and chat surface where retrieval-augmented answers stay grounded in my resume and project corpus. Production stays on Vercel; staging runs Dockerized on a local Proxmox Ubuntu VM behind a self-hosted GitHub Actions runner with quality gates and RAG sync before compose up.',
      features: [
        ...
        'Docker multi-stage image + Compose staging deploy on Proxmox.',
        'Self-hosted Actions workflow: checkout → lint/typecheck → RAG sync → docker compose up.',
      ],
      implementation: [
        ...
        'Authored Dockerfile (deps/builder/runner stages, Next standalone) and Compose service with healthchecks and resource limits.',
        'Wired deploy-staging.yaml on a self-hosted Linux runner: quality checks, secret-backed .env.staging, rag:sync gate, then docker compose up --build -d.',
        'Debugged Linux DNS host conflicts across vNICs, Windows NAT routing to the VM, and Docker port bindings so staging stayed reachable.',
      ...
      challenges: [
        ...
        'Homelab networking (DNS, multi-vNIC, Windows NAT, Docker ports) created brittle staging reachability until carefully documented.',
      ...
      reflection: [
        ...
        'Wiring real CI/CD onto a Proxmox Ubuntu VM taught me more about Linux networking and Docker than a dozen tutorials.',
  ...
  {
    id: 'homelab-cicd',
    title: 'Homelab Staging Lab',
    role: 'Systems / DevOps',
    summary:
      'Proxmox + Ubuntu VM lab: Dockerized portfolio staging, self-hosted GitHub Actions, and networking troubleshooting.',
    bullets: [
      'Run Proxmox hypervisor hosts and Ubuntu VMs for containerized staging of this portfolio.',
      'Self-hosted Actions runner gates lint/typecheck + RAG sync before docker compose rebuilds.',
      'Worked through DNS/vNIC conflicts, Windows NAT routing, and Docker port bindings end to end.',
    ],
    techStack: ['Proxmox', 'Ubuntu', 'Docker', 'GitHub Actions', '✚'],
    date: '2025 – Present',
    github: 'https://github.com/ttn64681/portfolio',
    exploreDossier: {
      overview:
        'Personal homelab focused on realistic deploy practice: Proxmox hypervisors, Ubuntu guests, Docker Compose staging for the RAG portfolio, and a self-hosted GitHub Actions runner that mirrors production-minded CI/CD without leaving the free/hobby budget.',
      features: [
        'Proxmox-managed VMs for isolated staging.',
        'Multi-stage Dockerfile + Compose healthchecks/resource limits.',
        'Self-hosted Actions: quality checks → RAG embedding sync → image rebuild.',
        'Documented networking path from Windows host through NAT to Linux guests.',
      ],
      implementation: [
        'Provisioned Ubuntu VMs under Proxmox and installed a self-hosted GitHub Actions runner.',
        'Authored deploy-staging.yaml to write secret-backed env files, run rag:sync, and docker compose up --build -d.',
        'Configured Compose logging rotation, CPU/memory limits, and HTTP healthchecks on the portfolio service.',
        'Resolved DNS host conflicts by separating vNIC responsibilities and validating Docker published ports.',
      ],
      challenges: [
        'DNS and multi-interface routing issues were harder than the Dockerfile itself.',
        'Windows host ↔ Linux VM NAT made “it works on my machine” debugging non-obvious.',
        'Keeping runner + secrets + Redis/Gemini credentials consistent across rebuilds.',
      ],
      reflection: [
        'Homelabbing scratched the same itch as game jams—shipping something that actually runs under constraints.',
        'I care about CI/CD because broken deploys waste more time than writing the feature.',
        'This lab is how I practice Linux, containers, and infra without an employer cloud budget.',
      ],
    },
  },
  {
  ...
  },
  {
    ...
    role: 'ML Researcher',
    summary:
      'Fine-tuned AST + HuBERT transformers (PyTorch/HF) to classify 10 bird species with strong accuracy on large and small sets.',
    bullets: [
      'Achieved 94%/91% accuracy on a large set (~5,000 samples) and 85%/69% on a small set (~1,500) via macro-F1 and confusion-matrix analysis.',
      'Gained ~1–4% accuracy with data augmentation, L2 regularization, and learning-rate scheduling.',
    ...
    exploreDossier: {
      overview:
        'Deep learning research project fine-tuning and evaluating AST and HuBERT transformer models with PyTorch/HuggingFace to classify 10 birds. Emphasis on reproducible training, honest evaluation across dataset sizes, and communicating comparative behavior.',
      features: [
        ...
        'Comparative reporting for overall and per-class behavior (macro-F1, confusion matrices).',
      ...
      implementation: [
        ...
        'Measured ~94/91% accuracy on ~5,000-sample (large) and ~85/69% on ~1,500-sample (small) splits.',
        'Improved accuracy ~1–4% via data augmentation, L2 regularization, and learning-rate scheduling.',
