...
export const portfolioDocuments: Document[] = [
  ...
  toDocument(
    'bio-summary',
    `Thai Nguyen is a senior Computer Science student at the University of Georgia (UGA) with a unique blend of \
    backend engineering skills and creative design passion. He specializes in full-stack architecture (Spring Boot, \
    Next.js) and AI integration (RAG, Gemini), but distinguishes himself through a deep commitment to meaningful UI/UX. \
    Thai doesn't just write code; he crafts cohesive user experiences, often leading the design process in Figma or \
    creating custom pixel art assets in Aseprite to ensure the final product feels polished and intentional.`,
  ...
  toDocument(
    ...
    creative UI/UX and digital art. He is a senior CS student at UGA graduating May 2026, with professional experience \
    in contract web development and a leadership role as CSIP Co-Chair for UGA ACM. He specializes in full-stack \
  ...
  toDocument(
    'education-uga',
    `Thai is pursuing a B.S. in Computer Science at the University of Georgia (Athens, GA), graduating in Aug 2026. He \
    holds a 3.70 / 4.0 major GPA (3.27 / 4.0 cumulative). Relevant coursework includes Data Structures, Algorithms, \
  ...
  toDocument(
    'leadership-acm',
    `Thai serves as an Executive Officer and CSIP Co-Chair for UGA ACM (May 2025 – Present). He leads weekly technical \
    workshops on Data Structures, Algorithms, and behavioral interview prep for 10+ students, and directs UI/UX redesigns \
    in Figma for club branding and digital presence. He also manages recruitment event logistics, helping increase student \
  ...
  toDocument(
    ...
    `In Fall 2025, Thai worked as a contract Full-Stack Developer for "Holywatr," a rock band. He engineered a multi-page \
    brand platform using vanilla JavaScript and CSS to meet a strict "digital-horror" aesthetic. Design & Art: Thai \
    implemented creative visual workarounds to match the band's gritty aesthetic, including custom assets and styling. \
    Tech: Integrated the Shopify Storefront API via customized GraphQL queries and implemented complex visual effects using \
    VFX.js (chromatic aberration, CRT distortion). UX: Solved performance bottlenecks using requestAnimationFrame loops to \
    maintain 60fps even with heavy shaders. Configured sitemap.xml and robots.txt for SEO, and implemented version \
  ...
  toDocument(
    'proj-cinema-backend',
    `For the "Actual Cinema Movies" project (Aug 2025 - Present), Thai architected a robust backend using Spring Boot 3.5 and \
    PostgreSQL. He designed 80+ RESTful API endpoints, managed 15 JPA entities, and implemented Caffeine caching to optimize \
    database loads. He also applied Facade and Virtual Proxy patterns to reduce API payloads by ~30-45%.`,
    {
  ...
  toDocument(
    'proj-cinema-ui',
    `Thai led the UI/UX design for the Cinema E-Booking project. He utilized Figma to prototype the interface and spearheaded \
    major UI revisions during the development phase. His focus was on creating a seamless booking flow—from movie browsing to \
    seat selection and payment—ensuring the complex backend logic was presented through an intuitive, user-friendly frontend.`,
    {
  ...
  toDocument(
    'proj-cinema-detail',
    `ACM Cinema (Actual Cinema Movies) is an end-to-end cinema management and booking platform: Aug 2025 – Present. Thai's role: \
    Scrum Master and Full-Stack Developer. Features: real-time seat selection, user and admin dashboards, secure payment \
    simulation. Security and auth: stateless sessions via JWT with custom JwtAuthenticationFilter and BCrypt password hashing. \
    Database: PostgreSQL with Flyway migrations and automated Bash backup scripts. Team: Led a 5-person team using Agile/Scrum; \
    improved sprint velocity by ~10% via structured Jira ticket management. The project was recognized as one of the top projects \
    in the Software Engineering course for its UI/UX and architectural depth.`,
    {
  ...
  toDocument(
    'proj-tower-ascent',
    `For "Tower Ascent" (Sep 2025 - Present), Thai serves as both a Programmer and Artist. Engineering: He designed a \
    componentized inventory UI using dependency injection to decouple display logic from game state. \
    Art & Animation: Thai uses Aseprite to create pixel art assets and animations for the game. He implements these assets\
    using Godot's AnimationPlayer, keyframing UI transitions and cutscenes to bring the pixel art to life. \
    He also implemented a modular trading system.`,
    {
  ...
  toDocument(
    'game-jams',
    `UGA Game Builders Club: Tower Ascent (Fall 2025 – Present): modular trading and inventory in Godot with Dependency \
    Injection; hand-animated UI and cutscenes in Aseprite. Bunker Jumper (Spring 2025): responsive UI and animations in Godot. \
    Laundry Day (Fall 2024): composed original music and SFX for a visual novel. To See (Spring 2024): composed the \
    soundtrack for a 3D storytelling experience. Game jams: Boss Rush Jam 2025 – "Dragon of the East" in Godot, music in \
    Garageband. Code for a Cause (charity jam) – "Job Description" in Unity with Blender and Photoshop; placed 41st out of 278. \
    A Forlorn Soul: placed 10th in Georgia; level design in Unity, art and animation in Aseprite, music in Garageband.`,
    {
  ...
  toDocument(
    'career-goals',
    `Thai is actively seeking roles in Software Engineering, AI Infrastructure, Cloud Engineering, and Full-Stack Development. \
    His interests are united by a "full-cycle" engineering philosophy: building high-performance backend systems (C/C++, Java, \
    Spring Boot) while maintaining an uncompromising standard for UI/UX and creative asset production. He believes meaningful \
    design is the key to software adoption and accessibility.`,
